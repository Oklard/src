...
class Controller_veiculo:
    ...
    def inserir_produto(self) -> veiculo:
        ...
        novo_produto = veiculo(df_produto.codigo_produto.values[0], df_produto.descricao_produto.values[0])
    ...
    def atualizar_produto(self) -> veiculo:
        ...
        if not self.verifica_existencia_produto(oracle, codigo_produto):
            ...
            df_produto = oracle.sqlToDataFrame(f"select codigo_produto, descricao_produto from produtos where codigo_produto = {codigo_produto}")
            # Cria um novo objeto Produto
            produto_atualizado = veiculo(df_produto.codigo_produto.values[0], df_produto.descricao_produto.values[0])
            # Exibe os atributos do novo produto
    ...
    def excluir_produto(self):
        ...
        if not self.verifica_existencia_produto(oracle, codigo_produto):            
            ...
            produto_excluido = veiculo(df_produto.codigo_produto.values[0], df_produto.descricao_produto.values[0])
